...
<strike>Примечание. Для перевода интерфейса модуля на русский язык необходимо активировать модуль `Locale` и импортировать файл `ru-ru.po` через меню `Конфигурация`-`Перевод интерфейса`- вкладка `Импорт`.</strike>
<strong>Теперь файл с переводом лежит в папке translations и эту процедуру при установке модуля можно не делать.</strong>
